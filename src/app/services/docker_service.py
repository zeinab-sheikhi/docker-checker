...
class DockerService(DockerServiceInterface):
        ...
        try:
            ...
            if performance is None:
                return DockerRunResponse(
                    success=False,
                    container_id=container.id,
                    error="Failed to read performance data"
                )
    ...
    def _read_performance_from_volume(self, volume: Volume) -> Optional[float]:
    ...
    def _cleanup_resources(self, container: Optional[Container], volume: Optional[Volume]):
